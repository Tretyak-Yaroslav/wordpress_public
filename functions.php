...
if ( ! function_exists( 'web_setup' ) ) :
	...
	function web_setup() {
		...
		add_theme_support( 'custom-background', apply_filters( 'web_custom_background_args', array(
...
function web_scripts() {
	wp_enqueue_style( 'web-style', get_stylesheet_uri() );

	wp_enqueue_script( 'web-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'web-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
...
//отключение автозамены в визуальном редакторе start
function wph_disable_mce_wptextpattern($opt) {
...
add_filter('tiny_mce_before_init', 'wph_disable_mce_wptextpattern');
//отключение автозамены в визуальном редакторе end